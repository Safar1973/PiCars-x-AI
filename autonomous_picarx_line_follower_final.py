...
# Diese Werte können angepasst werden, um die Erkennung zu verbessern
BROWN_LOWER = np.array([5, 50, 50])    # Unterer HSV-Bereich für Braun
BROWN_UPPER = np.array([30, 255, 150]) # Oberer HSV-Bereich für Braun
    ...
    try:
        ...
        try:
            ...
            if hasattr(Vilib, 'color_dict'):
                # HSV-Bereiche für Braun: H=[5-30], S=[50-255], V=[50-150]
                Vilib.color_dict['brown'] = [[5, 30], [50, 255], [50, 150]]
        ...
        try:
            ...
            print("Braune Farberkennung aktiviert.")
        except Exception as e:
            print(f"Fehler bei der Aktivierung der Farberkennung: {e}")
            # Fallback zur roten Farbe, falls braun nicht funktioniert
            try:
                Vilib.color_detect("red")
                print("Fallback: Rote Farberkennung aktiviert.")
            except:
                print("Auch rote Farberkennung fehlgeschlagen.")
    ...
    try:
        ...
        if hasattr(Vilib, 'img') and Vilib.img is not None:
            ...
            if len(contours) > 0:
                ...
                # Nur fortfahren, wenn die Kontur groß genug ist
                if cv2.contourArea(c) > 100:  # Mindestgröße anpassen
                    ...
                    if M["m00"] > 0:
                        ...
                        line_position = (cx - width/2) / (width/2)
                        line_detected = True
...
def follow_line_forward(distance_cm, speed=20):
...
def follow_line_backward(distance_cm, speed=15):
...
def main():
    ...
    try:
        ...
        # Nach der Drehung kurz warten und prüfen, ob die Linie erkannt wird
        print("Suche Linie nach der Drehung...")
        line_found = False
        for _ in range(10):  # 10 Versuche
            line_found, _ = detect_line()
            if line_found:
                print("Linie nach der Drehung erkannt!")
                break
            sleep(0.5)
        
        if not line_found:
            print("WARNUNG: Keine Linie nach der Drehung erkannt. Versuche trotzdem fortzufahren.")
